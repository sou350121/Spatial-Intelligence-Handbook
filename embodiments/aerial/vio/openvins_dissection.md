...
|---|---|---|---|---|
| 1 | Filter diverges after init even with clean Kalibr (≤0.5 px reproj err); trajectory blows up from ~900 m to >2000 m on ZED-X stereo + IMU | [issue #540](https://github.com/rpng/open_vins/issues/540): "The system starts normally and begins processing data, but shortly after initialization the estimated trajectory diverges rapidly" (2026-03) — user re-ran Kalibr multiple times, still diverges | 🔴 | Don't trust Kalibr reproj-err alone; verify IMU axis convention + `T_imu_cam` sign by hand; cross-check gravity_mag against site latitude |
| 2 | Aerial high-altitude (~100 m) rapid divergence post-init with self-collected UAV bag | [issue #513](https://github.com/rpng/open_vins/issues/513): "self-collected UAV flight dataset at an altitude of around 100m … After successful initialization, the navigation solution diverges very quickly" (2025-06, RFU IMU frame, `max_clones:11`, `max_slam:50`) | 🔴 | Large-parallax scenes need wider clone window + tighter feature gating; 11 clones is EuRoC-tuned, raise to 15–20 for >50 m altitude |
| 3 | Instant segfault on Jetson Orin Nano with D435i + D455 preset; T265 preset OK | [issue #514](https://github.com/rpng/open_vins/issues/514): "I am trying to use openvins with d435i, and have calibrated all sensors. The config file gives me an instant segfault" (2025-06, exit -11; suspect OpenCV 4.12 + CUDA) | 🟠 | Pin OpenCV to 4.5.x without CUDA on Orin Nano; start from working T265 preset and migrate fields one-by-one |
| 4 | Race condition in `_clones_IMU` cache → undefined behavior under multi-thread load | [issue #531](https://github.com/rpng/open_vins/issues/531): `StateHelper::augment_clone` writes to `state->_clones_IMU[state->_timestamp]` while `margtimestep()` reads same map without lock; closed 2025-12 after GDB trace | 🟠 | Update to post-2025-12 master (PR merged); if pinned to older release, serialize propagate/update from same thread |
| 5 | RealSense D435i severe trajectory divergence on ROS2 Humble | [issue #535](https://github.com/rpng/open_vins/issues/535): "Severe trajectory divergence with Realsense D435i on ROS2 Humble" (closed 2026-01) — root cause was QoS / sync mismatch | 🟠 | Use SensorDataQoS for IMU topic; verify D435i firmware ≥5.13; cross-check with `ros2 topic hz` before launch |
| 6 | Multi-camera non-overlapping rigs (e.g. 3 front + 3 rear) need motion excitation no aerial pre-flight provides | [issue #534](https://github.com/rpng/open_vins/issues/534): "add 6 monocular cameras using this repo? … 3 cameras in front making a trinocular vision and 3 at back" (2026-01) — rear-only cams unobservable in forward flight | 🟡 | Either offline-calibrate non-overlapping extrinsics with Kalibr, or scripted figure-8 yaw maneuver pre-flight to excite each cam's extrinsic |
| 7 | YAML config / actual IMU sample-rate mismatch silently corrupts state | [issue #514](https://github.com/rpng/open_vins/issues/514) thread + recurring `static init fail` reports (e.g. [#477](https://github.com/rpng/open_vins/issues/477)) — config assumes nominal 200 Hz, hardware delivers 180 Hz with jitter | 🟡 | Measure actual IMU `rostopic hz` for 60 s before flight; set `imu_rate` to measured value, not datasheet value |
| 8 | ROS 2 port still trailing master features; `pathgt` empty, `save_total_state` broken | [issue #502](https://github.com/rpng/open_vins/issues/502) (2025-04, `pathgt` empty) + [issue #477](https://github.com/rpng/open_vins/issues/477) (2024-10, state save broken) — both still open | 🟡 | Apply [PR #539](https://github.com/rpng/open_vins/pull/539) (ROS2 save_total_state fix) and [PR #543](https://github.com/rpng/open_vins/pull/543) (ROS2 header fixes) locally |

**Repo health signal**: 2.9k★ / 60 open / ~290 closed / last commit 2025-11-30 (master), 8 open PRs all 2024-10→2026-04 (active triage). Maintainer goldbattle merges PRs but few inline replies on user-platform issues; **the most actively maintained aerial VIO repo in this section.**

**讀者實務含義**: OpenVINS 在 2026-05 仍是「最值得選的開源 aerial VIO」，但活躍維護不等於「拿來就跑」——上面 8 條都是已被多用戶複現的真實坑。實務序：先在 EuRoC bag 上跑通 →用 T265 preset 驗 sensor stack →再用 Kalibr 校自己的 D435i/ZED-X →最後上飛機。Jetson Orin Nano + D435i 是高踩坑率組合（#514 / #533 / #535 三案皆此 stack）；高空（>50 m）必須調 `max_clones`。多相機非重疊配置（#534）需要地面 figure-8 預激勵——這是 Skydio「pre-flight wiggle」的開源對應。
