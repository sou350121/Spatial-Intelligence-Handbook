...
本文覆盖 *板载* 空中建图——飞行时跑在无人机上的部分。地面站后处理（全质量 3DGS、Pix4D / RealityCapture 摄影测量流水线）不在范围。Per-method 剖析（MonoGS、FAST-LIO2 等）在 [`foundations/`](../../../foundations/)。跨实体的"何时 3DGS 胜 NeRF 胜经典深度"在 [`foundations/3dgs-family/`](../../../foundations/3dgs-family/) 与 [`crossing/`](../../../crossing/)。Long-range SLAM 在此吸收而不拆出 `long-range-slam/`——compute 与 GNSS-denial 约束与建图选择不可分。Sensor-stack 权衡（LiDAR 档、stereo baseline）在 [`../sensor-stack/`](../sensor-stack/)。
