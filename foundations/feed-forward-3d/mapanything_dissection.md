...
> **Scope:** 直接读 `facebookresearch/map-anything` 2026-01 → 2026-05 issue / PR / release 流，校验 §6.3.x 之外的真实部署痛点。比 VGGT v1 早期成熟度更低（issue 量少、回应也少），但 PR 节奏更快。
> **Repo health (2026-05-24):** ~3.4k★, **28 open issues**, **0 open PRs / 30 closed**（合 PR 速度明显比 VGGT 健康）；License **CC-BY-NC-4.0**（非商用）。最新版本 **v1.1.1**（2026-03-23 release，含 Pixio encoder + MPS 支持）。

### 7.5.1 — 已落地痛点表

| Issue | 标题 / 核心引文 | 严重度 | Workaround |
|---|---|---|---|
| [#147](https://github.com/facebookresearch/map-anything/issues/147) | "While I was initially pleased with MapAnything results... I have yet been quite deceived with the reprojection consistency" — `YanNoun`（labels: enhancement + question）| **High**（亚厘米精度场景致命）| 点云**看似一致**但 per-view 反投影错位；manipulation grasp / drone control 需自测 reprojection error，不能只信 metric loss。当前 factored repr 跨视图 3D 一致性弱于 MASt3R。 |
| [#155](https://github.com/facebookresearch/map-anything/issues/155) | "publicly available ASE dataset contains 100,000 scenes, yet the released metadata files total 103,890 scenes... ase_scene_list_train.npy: 98,696 / val: 5,194" — `seanmu` | **Med**（finetune blocker）| 官方 metadata 与公开 ASE 不匹配；finetune 前需手动 align scene IDs。 |
| [#153](https://github.com/facebookresearch/map-anything/issues/153) | "WAI conversion about the ase dataset"（文档 gap） | **Med**（custom data 接入工程债）| 数据转换脚本不完整，需自行补 WAI 格式 conversion。 |
| [#152](https://github.com/facebookresearch/map-anything/issues/152) | "How to open the large glb?"，500 张图生成 **2.4 GB GLB / 6.4 GB PLY**，"125M → 5M faces decimation 后仍崩" — `monajalal` | **Med-High**（输出可用性）| 原生输出**不下采样**；下游需走 decimation / voxel downsample / supersplat 类 viewer。Blender / MeshLab 直开不可行。 |
| [#150](https://github.com/facebookresearch/map-anything/issues/150) | "I have the exact extrinsics (Rt) and intrinsics (K)... I observe instead that these input parameters are considered as an 'optimization initialization' as they are modified during the inference process" — `FrancescoFornasa` | **High**（multi-camera 语义 violated）| Optional 输入并非 ground-truth 锁定，模型仍会"调"。**已知精确标定时，当前 API 无法 freeze K/T。** §6.2 第 3 条假设由此 GitHub-validated。 |
| [#141](https://github.com/facebookresearch/map-anything/issues/141) | "External predictions with `mapanything` model not working... `init_model_from_config(args.model_name, device=device)` without properly loading model weights from the checkpoint" — `omaralvarez` | **Med**（pipeline 集成 blocker）| 走 `MapAnything.from_pretrained("facebook/map-anything")` 而不是 `init_model_from_config`，前者正确加载权重。 |
| [#149](https://github.com/facebookresearch/map-anything/issues/149) | "It forces weights download for `dinov2_vitg14` from torch hub. It's redundant, as the weights are already included in hugging face checkpoint" — `barwojcik` | **Low-Med**（离线 / air-gap 部署痛点）| `DINOv2Encoder` 强制从 torch hub 拉权重；离线机器初始化会卡住，需手动 pre-cache `~/.cache/torch/hub/`。已分流到上游 UniCeption。 |

### 7.5.2 — Release / PR 信号

| 项 | 内容 | 含义 |
|---|---|---|
| **v1.1** (2026-01-18, PR [#130](https://github.com/facebookresearch/map-anything/pull/130)) | 新 checkpoints + **AerialMegaDepth dataset** integration | aerial 用例正在被一等公民对待；和手册 §8 的"aerial 慢速巡检"判断一致。 |
| **v1.1.1** (2026-03-23) | + Pixio encoder (PR [#151](https://github.com/facebookresearch/map-anything/pull/151)) + **MPS inference** (PR [#131](https://github.com/facebookresearch/map-anything/pull/131)) | Apple Silicon 路径打通；macOS 本地测试 / iPhone capture pipeline 可行性上升。**v1.1.2 在 GitHub 上未见**，与任务清单描述存在偏差。 |
| **0 open PRs** | 30 PR 全合 / 关 | PR review 节奏远健康于 VGGT v1（24 open）。 |
| **License (CC-BY-NC-4.0)** | 模型权重非商用 | GitHub 上**几乎没有商用许可相关 issue**——可能因为商用用户直接绕开，也可能因为权重未触发主流商用尝试。**Apache 2.0 ≠ 权重 license**：repo 代码 Apache 2.0 ≠ HF checkpoint 是 CC-BY-NC-4.0。这是一处常见误读，建议读者**在用任何 checkpoint 前直接看 HF 的 license 字段**。 |

### 7.5.3 — Repo health & 读者实务含义

- **成熟度判定**: 比 VGGT v1 *新得多*（半年生命周期 vs 14 月），但 **PR 合入速度健康**、release cadence 清晰（v1.0.1 → v1.1 → v1.1.1 三个月内）。这是上升期 repo 的典型形态；建议**追主分支**而非锁版本。
- **对手册 §6.3.x 的反馈**:
  1. §6.3.x 的 [#147](https://github.com/facebookresearch/map-anything/issues/147)（reprojection consistency）经 OP 描述确认是 **"point cloud 看似完美 → per-view 反投影错位"**——典型 *silent geometric failure*。此条应升 §6.2 hidden assumption "Factored heads 互不干扰" 的反例：实际 4 个 head 之间**确实存在 trade-off**，metric loss 收敛 ≠ geometry consistent。
  2. §6.2 第 3 条 "Optional inputs 不会让模型耍懒" 被 [#150](https://github.com/facebookresearch/map-anything/issues/150) **直接证伪反向**——模型不仅"信任输入"，反而*改输入*。**multi-camera 已标定场景下不可直接使用** —— 这对 robotics rig（D435 + 已知 base-to-camera transform）是真正的 deal-breaker。
  3. §3.1 训练 dataset list 的 UNVERIFIED 缺口被 [#155](https://github.com/facebookresearch/map-anything/issues/155) + [#153](https://github.com/facebookresearch/map-anything/issues/153) 进一步暴露——连官方 metadata 与公开 ASE 的 scene 数都对不上（100k vs 103.89k）。复现 paper 的训练几乎不可行。
- **新增 §6.x 候选条目**:
  - GLB 输出膨胀：500 张图 → 2.4 GB GLB（[#152](https://github.com/facebookresearch/map-anything/issues/152)），生产 pipeline 必须接 decimation。
  - DINOv2 强制 torch hub 下载（[#149](https://github.com/facebookresearch/map-anything/issues/149)），air-gap 部署需 pre-cache。
  - License 双重性：repo code Apache 2.0，HF checkpoint CC-BY-NC-4.0；§4.1 表格应明示 "**weights 非商用，code 商用**"。
- **对 aerial 实务（§8）的微调**: AerialMegaDepth 已合入主分支 → "慢速巡检 hybrid" 推荐**更进一步**；但 [#150](https://github.com/facebookresearch/map-anything/issues/150) 揭示已标定相机数据无法 freeze 输入 → drone 上 *固定 baseline stereo* 的 calibration 优势暂时白费，待官方加 `freeze_K=True` flag。
