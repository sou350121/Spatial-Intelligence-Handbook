...
### Kalibr GitHub 实地失败（atlas 联动）

- **GitHub-validated**：**"必须用但没人修"** — 5.4k★ / 133 open issues / 最近 push 2024-03，官方维护停滞但**仍是事实标准**（无替代品），社区围绕 ROS 2 port 自治；这是 atlas 单独点出的最高工具锁定 risk，详见 [`github_failure_atlas.md`](./github_failure_atlas.md)。
- **GitHub-validated**：**收敛失败 / 外参跑出离谱数字**（#756 / #768 / #765）根因正是上文实战 caveat 的两条：6-DoF 激发不足 + IMU noise 用 datasheet 而非 allan variance；自打印 AprilTag 边界污染 (#760 / #759) 是另一隐患，详见 [`github_failure_atlas.md`](./github_failure_atlas.md)。
- **GitHub-validated**：**不要指望官方接 ROS 2** — #725 "upgrade to ROS 2" (6 reactions, 7 comments) 自 2024 stale；上文推荐的"ROS 1 容器隔离 Kalibr"是社区共识，详见 [`github_failure_atlas.md`](./github_failure_atlas.md)。
...
### maplab GitHub 实地失败（atlas 联动）

- **GitHub-validated**：Ubuntu 22.04 build 系列失败 (#422 / #413 / #411) + maplab 2.0 rovioli front-end 跑不通 (#414 / #389) 印证"工业用得不多、文档零碎、ROS 2 迁移滞后"；ETHZ-ASL 团队大量转 Skydio / 工业 → 维护稀疏，详见 [`github_failure_atlas.md`](./github_failure_atlas.md)。
- **GitHub-validated**：**LiDAR mapping 是用户多年请求** (#412 / #417 / #365) — 学术 repo 永不跨这步，工业方向需自己重做；这是上文"工业自研 mapping backend"现实的 issue tracker 印证，详见 [`github_failure_atlas.md`](./github_failure_atlas.md)。
