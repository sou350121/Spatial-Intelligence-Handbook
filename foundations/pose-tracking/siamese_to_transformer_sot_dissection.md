...
- **GitHub-validated**：SAM 2 是这条 SOT 演进的当代终点，其 issue 区暴露 memory bank 是隐藏 hyperparameter — 2 小时 720p/60fps 在 4090 OOM（[#264](https://github.com/facebookresearch/sam2/issues/264)·[#90](https://github.com/facebookresearch/sam2/issues/90)），video 中途加新 ID 难（[#249](https://github.com/facebookresearch/sam2/issues/249)·[#185](https://github.com/facebookresearch/sam2/issues/185)），input 分辨率变 mask shape mismatch（[#138](https://github.com/facebookresearch/sam2/issues/138)），TRT/ONNX 导出 memory_attention 模块产生 NaN（[#284](https://github.com/facebookresearch/sam2/issues/284)·[#186](https://github.com/facebookresearch/sam2/issues/186)）；本 dissection §6 "Memory bank 长度有限" 假设在 issue 区落地为 streaming + memory pruning 的 PR 焦点，详见 [`github_failure_atlas.md`](./github_failure_atlas.md)
- **GitHub-validated**：Siamese / Transformer SOT 经典作品（SiamFC / STARK / OSTrack）本身不在 atlas 5 仓直接覆盖范围；atlas 5 仓选了 FoundationPose / MegaPose / RAFT / CoTracker / SAM 2 作为 pose & tracking foundation 阵营代表，**经典 SOT 在 atlas 中作为"被 SAM 2 革命取代的祖辈"被讨论**，未有专属失败块；详见 atlas zone summary 与 SAM 2 失败块。
